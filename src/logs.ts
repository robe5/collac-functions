...
export function emailSended(uid: string, type: string) {
  logger.log(`✉️ [Mail]: Sended email to user [${uid}] of type [${type}]`);
}

export function emailIgnored(uid: string, type: string) {
  logger.log(`✉️ [Mail]: Ignored email to user [${uid}] of type [${type}]`);
}
